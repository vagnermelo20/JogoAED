...
void unload_all_pilha(Pilha** head) {
	// Implementar a libera��o de mem�ria
	Pilha* atual = *head;
	Pilha* proximo;
	while (atual != NULL) {
		proximo = atual->next;
		free(atual->carta); // Libera a carta
		free(atual);        // Libera o n� da pilha
		atual = proximo;
	}
...
// Função auxiliar para criar e adicionar cartas
...
void initialize_baralho(Pilha** baralho, int num_cartas) {
    ...
    for (int c = 0; c < 4; c++) {
        ...
        // Adiciona duas cópias do resto (1-9, +2, Bloquear, Reverso)
        for (int i = 0; i < 2; i++) {
